...
class OpenAIHelper:
  ...
  async def get_chat_response(self, chat_id: int,
  ...
  async def get_chat_response_stream(self, chat_id: int, query: str):
    """
        ...
        """
    response = await self.__common_get_chat_response(chat_id,
                                                     query,
                                                     stream=True)
  ...
  async def __common_get_chat_response(self,
    ...
    try:
      ...
      exceeded_max_tokens = token_count + self.config[
        'max_tokens'] > self.__max_model_tokens()
  ...
  async def generate_image(self, prompt: str) -> tuple[str, str]:
    """
        ...
        """
    try:
      response = await openai.Image.acreate(prompt=prompt,
                                            n=1,
  ...
  async def __summarise(self, conversation) -> str:
    ...
    messages = [{
      ...
      "assistant",
      "content":
      "Summarize this conversation in 700 characters or less"
    }, {
      "role": "user",
      "content": str(conversation)
    }]
    response = await openai.ChatCompletion.acreate(model=self.config['model'],
                                                   messages=messages,
                                                   temperature=0.4)
  ...
  def get_billing_current_month(self):
    ...
    response = requests.get("https://api.openai.com/dashboard/billing/usage",
                            headers=headers,
                            params=params)
    billing_data = json.loads(response.text)
    usage_month = billing_data[
      "total_usage"] / 100  # convert cent amount to dollars
    return usage_month
