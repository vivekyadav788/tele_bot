...
def message_text(message: Message) -> str:
  ...
  for _, text in sorted(message.parse_entities([MessageEntity.BOT_COMMAND
                                                ]).items(),
...
class ChatGPTTelegramBot:
  ...
  def __init__(self, config: dict, openai: OpenAIHelper):
    ...
    self.commands = [
      ...
      BotCommand(
        command='reset',
        description=
        'Reset the conversation. Optionally pass high-level instructions '
    ...
    self.restricted_message = "Sorry, you are not allowed to use this feature."
    self.usage = {}
    self.last_message = {}
  ...
  async def stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    transcribe_durations = self.usage[
      user_id].get_current_transcription_duration()
  ...
  async def resend(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in self.last_message:
      ...
      await context.bot.send_message(chat_id=chat_id,
                                     text="You have nothing to resend")
      return
  ...
  async def image(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not self.is_admin(update):
      await self.send_restricted_message(update, context)
      return
    ...
    if image_query == '':
      await context.bot.send_message(
        chat_id=chat_id, text='Please provide a prompt! (e.g. /image cat)')
      return
    ...
    await self.wrap_with_indicator(update, context,
                                   constants.ChatAction.UPLOAD_PHOTO,
                                   _generate)

  async def transcribe(self, update: Update,
    ...
    async def _execute():
      ...
      try:
        ...
        await context.bot.send_message(
          ...
          reply_to_message_id=update.message.message_id,
          text=
          f'Failed to download audio file: {str(e)}. Make sure the file is not too large. (max 20MB)',
          parse_mode=constants.ParseMode.MARKDOWN)
        return
    ...
    await self.wrap_with_indicator(update, context,
                                   constants.ChatAction.TYPING, _execute)
  ...
  async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
      if self.config['stream']:
        ...
        async for content, tokens in stream_response:
          ...
          if len(chunks) > 1:
            ...
            if chunk != len(chunks) - 1:
              chunk += 1
              try:
                await self.edit_message_with_retry(context, chat_id,
          ...
          elif abs(len(content) -
                   len(prev)) > cutoff or tokens != 'not_finished':
            prev = content
        ...
        await self.wrap_with_indicator(update, context,
                                       constants.ChatAction.TYPING, _reply)
  ...
  async def inline_query(self, update: Update,
                         context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    ...
    results = [
      InlineQueryResultArticle(
        ...
        description=query,
        thumb_url=
        'https://user-images.githubusercontent.com/11541888/223106202-7576ff11-2c8e-408d-94ea-b02a7a32149a.png'
      )
    ]

    await update.inline_query.answer(results)

  async def edit_message_with_retry(self,
                                    context: ContextTypes.DEFAULT_TYPE,
                                    chat_id: int,
                                    message_id: int,
                                    text: str,
  ...
  async def wrap_with_indicator(self, update: Update, context: CallbackContext,
                                chat_action: constants.ChatAction, coroutine):
    """
  ...
  async def send_budget_reached_message(self, update: Update,
                                        context: ContextTypes.DEFAULT_TYPE):
    """
  ...
  async def send_restricted_message(self, update: Update,
                                    context: ContextTypes.DEFAULT_TYPE):
    """
        Sends the restricted message to the user.
        """
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                   text=self.restricted_message)
  ...
  async def is_user_in_group(self, update: Update, context: CallbackContext,
                             user_id: int) -> bool:
    """
        Checks if user_id is a member of the group
        """
    try:
      chat_member = await context.bot.get_chat_member(update.message.chat_id,
                                                      user_id)
      return chat_member.status in [
        ChatMember.OWNER, ChatMember.ADMINISTRATOR, ChatMember.MEMBER
      ]
    except telegram.error.BadRequest as e:
    ...
    if self.is_group_chat(update):
      ...
      for user in itertools.chain(allowed_user_ids, admin_user_ids):
        if await self.is_user_in_group(update, context, user):
          ...
          return True
      logging.info(
        f'Group chat messages from user {update.message.from_user.name} '
  ...
  async def is_within_budget(self, update: Update,
    ...
    if self.is_group_chat(update):
      ...
      for user in itertools.chain(allowed_user_ids, admin_user_ids):
        if await self.is_user_in_group(update, context, user):
          ...
          if self.config['monthly_guest_budget'] >= self.usage[
              'guests'].get_current_cost()[1]:
            return True
  ...
  def run(self):
    ...
    application.add_handler(
      MessageHandler(
        filters.AUDIO | filters.VOICE | filters.Document.AUDIO | filters.VIDEO
        | filters.VIDEO_NOTE | filters.Document.VIDEO, self.transcribe))
    application.add_handler(
